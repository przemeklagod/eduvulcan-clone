...
import type { Exam } from '@/src/api/hebe/types/exam';
import { useExams } from '@/src/data/useExams';
import { useThemeColors } from '@/src/ui/theme';
import { formatHebeDate } from '@/src/utils/dates';
import { htmlToPlainText } from '@/src/utils/richText';

function sortByDeadline(items: Exam[]): Exam[] {
  return [...items].sort((a, b) => a.DeadlineAt.localeCompare(b.DeadlineAt));
}

export default function ExamsScreen() {
  const colors = useThemeColors();
  const { exams, isLoading, isRefetching, error, refetch, hasActiveStudent } = useExams();
  const rows = useMemo(() => sortByDeadline(exams), [exams]);
    ...
    return (
      <View style={styles.center}>
        <Text style={[styles.error, { color: colors.danger }]}>
          {error instanceof Error ? error.message : 'Błąd ładowania sprawdzianów'}
        </Text>
      </View>
    );
  }
    ...
    <FlatList<Exam>
      ...
      renderItem={({ item }) => (
        <View style={[styles.card, { borderBottomColor: colors.border }]}>
          ...
          {item.Content && <Text style={[styles.content, { color: colors.text }]}>{htmlToPlainText(item.Content)}</Text>}
          <Text style={[styles.creator, { color: colors.secondaryText }]}>{item.Creator.DisplayName}</Text>
        </View>
      )}
      ListEmptyComponent={
        <View style={styles.center}>
          <Text style={{ color: colors.text }}>Brak zaplanowanych sprawdzianów.</Text>
        </View>
      }
    />
  );
